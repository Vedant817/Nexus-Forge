...
async function purgeProjectContent(tx: Parameters<Parameters<typeof prisma.$transaction>[0]>[0], projectId: string): Promise<void> {
  ...
  await tx.findingResolution.deleteMany({ where: { finding: { projectId } } })
  await tx.findingWaiver.deleteMany({ where: { finding: { projectId } } })
  await tx.findingComment.deleteMany({ where: { finding: { projectId } } })
